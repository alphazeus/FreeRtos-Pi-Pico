...
void play_tone(uint8_t pin, uint16_t frequency, uint16_t duration_ms) {
...
void vPlaySongTask() {
    ...
    while (true) {
        for (int i = 0; i < sizeof(ttls)/sizeof(ttls[0]); i++) {
            ...
            play_tone(BUZZER_PIN, note_frequencies[note_index], duration);
        }

        // Play each note
        // for (size_t i = 0; i < sizeof(note_frequencies) / sizeof(note_frequencies[0]); i++) {
        //     play_tone(BUZZER_PIN, note_frequencies[i], 500);
        // }
    }
}
